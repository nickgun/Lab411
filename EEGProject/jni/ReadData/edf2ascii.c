...
{
  mat _X;
  _X = mat_create(SAMPLE_MAX,CHANNEL_NUMBER);
...
/***************** write header ******************************/
/*tantn:bo ham nay de xu ly tren android k lag
...
/* done with timekeeping and annotations, continue with the data */
    ...
    do
    {
	  ...
	  //tantn: neu gia tri value_tmp la cua 14 kenh headset >> luu vao mang eegData[][]
	  if((j>=2)&&(j<=15)){
	    if(eegDataSampleCount<=128)
	      _X[eegDataSampleCount][j-2] = value_tmp; 
	  }
  ...
  mat_delete(_X, SAMPLE_MAX, CHANNEL_NUMBER);
